...
use crate::{accumulate::TranscriptSegment, build_summary_message, ChatMsg, SessionRecord, SessionStore};
...
pub async fn finalize_inner(
    ...
    if settings.use_local_summary {
        ...
        let message = build_summary_message(&record.segments, screen_context.as_deref());
        if let Err(err) = host
            .summarize_local(relay_session, message, user_message_id)
            .await
        {
            finalize_error(host, store, &mut record, relay_session.to_string(), &err);
        }
        return;
    }
