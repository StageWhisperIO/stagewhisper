...
pub fn start_local_capture(app: AppHandle) -> Result<AudioStopSignal, String> {
    ...
    crate::logging::set_active_session(&session_id_str);
    {
...
fn run_local_capture(
    ...
    *startup_notified = true;
    ...
    let mic_stream = match (mic_pipeline.is_some(), mic_audio_tx) {
        (true, Some(tx)) => {
            let dropped_mic_cb = Arc::clone(&dropped_mic);
            match build_mic_input_stream(move |chunk: Vec<i16>| {
                if tx.try_send(AudioFrame::Mic(chunk, Instant::now())).is_err() {
                    let dropped = dropped_mic_cb.fetch_add(1, Ordering::Relaxed) + 1;
                    if dropped % 50 == 1 {
                        eprintln!(
                            "[audio-local] microphone capture queue full; dropped {dropped} chunks"
                        );
                    }
                }
            }) {
            ...
            Ok(frame) => match frame {
                AudioFrame::System(chunk, captured_at) => {
                    ...
                    if system_capture_abandoned {
                        ...
                        eprintln!(
                            "[audio-local] system audio recovered; resuming stall monitoring"
                        );
                    }
    ...
    if let Some(ref mut mic) = mic_pipeline {
        if let Some(ref mut ec) = echo_canceller {
            ...
            if !near.is_empty() {
                let events = mic.process_audio_chunk(&near);
                dispatch_events(app, &events, accumulator, collector, TranscriptSource::You);
            }
        }
...
fn relabel_segments(
    ...
    let utterances: Vec<sw_audio_recording::diarization::Utterance> = pending_utterances
        .iter()
        .map(
            |(index, samples)| sw_audio_recording::diarization::Utterance {
                index: *index,
