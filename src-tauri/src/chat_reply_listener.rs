...
fn is_notes_turn_settled(
    ...
    matches!(status, "completed" | "errored" | "cancelled" | "silent")
        && notes_root_message_id.is_some()
...
mod tests {
    ...
    #[test]
    fn auto_title_only_on_notes_turn_not_on_follow_up_replies() {
        let notes_root = Some("notes-root");
        assert!(is_notes_turn_settled("completed", notes_root, Some("notes-root")));

        assert!(!is_notes_turn_settled("completed", notes_root, Some("chat-msg-2")));
        assert!(!is_notes_turn_settled("message", notes_root, Some("notes-root")));
        assert!(!is_notes_turn_settled("typing", notes_root, Some("notes-root")));
        assert!(!is_notes_turn_settled("completed", None, Some("notes-root")));
        assert!(!is_notes_turn_settled("completed", notes_root, None));
    }
