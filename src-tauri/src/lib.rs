...
fn resolve_advertised_callback(
    ...
    if pairing::relay_is_loopback(&settings.relay_url) {
        return loopback;
    }
...
fn env_callback_is_configured(env_callback_url: Option<String>) -> bool {
    let Some(raw) = env_callback_url else {
        return false;
    };
    let trimmed = raw.trim().trim_end_matches('/');
    !trimmed.is_empty() && pairing::validate_relay_url(trimmed).is_ok()
}

#[tauri::command]
fn callback_env_configured() -> bool {
    env_callback_is_configured(std::env::var("STAGEWHISPER_CALLBACK_URL").ok())
}
...
fn probe_indicates_reachable(status: &str) -> bool {
...
fn spawn_local_reply(
    ...
    let queue = app.state::<state::local_llm::LocalTurnQueue>();
    let app = app.clone();
    let queue_key = session_id.clone();
    queue.enqueue_for(&queue_key, async move {
        let Ok(store) = session_store(&app) else {
            return;
        };
        let record = store.load(&session_id).ok().flatten();
        let system = sw_local_llm::system_prompt_with_summary(
            LOCAL_LLM_SYSTEM_PROMPT,
            record.as_ref().and_then(|r| r.notes_markdown.as_deref()),
        );
        let entries = record.map(|record| record.chat).unwrap_or_default();
        let mut history = sw_local_llm::history_before(
            &entries
                .iter()
                .map(|m| sw_local_llm::StoredMessage {
                    id: &m.id,
                    role: &m.role,
                    status: &m.status,
                    content: &m.content,
                    parent_message_id: m.parent_message_id.as_deref(),
                })
                .collect::<Vec<_>>(),
            &parent_id,
        );
        history.push(sw_local_llm::ChatMessage::user(prompt));
        let history = sw_local_llm::prepare_chat_history(
            &history,
            sw_local_llm::CHAT_HISTORY_MAX_MESSAGES,
            sw_local_llm::CHAT_HISTORY_MAX_CHARS,
        );

        let result =
            state::local_llm::generate_reply_messages(&app, Some(&system), &history).await;
        let now = chrono::Utc::now().to_rfc3339();
...
pub fn run() {
    let app = tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            state::local_llm::local_turn_pending,
        ...
        .setup(|app| {
            let initial_models_ready =
                state::local_pipeline::all_models_ready(&sw_audio_recording::download::default_model_dir());
            ...
            tauri::async_runtime::spawn(async move {
                ...
                {
                    Ok(listener) => {
                        let callback_url = resolve_advertised_callback(
                            &settings,
                            std::env::var("STAGEWHISPER_CALLBACK_URL").ok(),
                            listener.local_port(),
                        );
...
mod tests {
    ...
    #[test]
    fn loopback_relay_overrides_stale_persisted_callback() {
        let settings = relay_settings::RelaySettings {
            relay_url: "http://127.0.0.1:8765".to_string(),
            callback_url: Some("https://stale.tail34b074.ts.net".to_string()),
            ..Default::default()
        };
        let advertised = resolve_advertised_callback(&settings, None, 8788);
        assert_eq!(advertised, "http://127.0.0.1:8788");
