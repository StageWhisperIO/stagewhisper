use super::*;
use tauri::Emitter;
use tauri::{Runtime, Size, WebviewUrl};
use tauri_nspanel::{
...
fn show_session_panel(app_handle: &AppHandle) {
    ...
    position_panel_below_control(&*control, &*session);
...
fn reposition_visible_session(app_handle: &AppHandle) {
    let (Ok(control), Ok(session)) = (
        app_handle.get_webview_panel(CONTROL_LABEL),
        app_handle.get_webview_panel(SESSION_LABEL),
    ) else {
        return;
    };
...
fn position_panel_below_control(
    ...
    unsafe {
        ...
        let control_frame: NSRect = msg_send![control.as_panel(), frame];
        let panel_frame: NSRect = msg_send![panel.as_panel(), frame];

        // In macOS coords (bottom-left origin), "below" means smaller Y
        let panel_origin = NSPoint::new(
            control_frame.origin.x - (panel_frame.size.width - control_frame.size.width) / 2.0,
            control_frame.origin.y - panel_frame.size.height - PANEL_GAP,
        );
