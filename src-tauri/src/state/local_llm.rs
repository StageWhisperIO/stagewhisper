...
fn llama_dir(app: &AppHandle) -> PathBuf {
    ...
    if let Ok(resource) = app.path().resource_dir() {
        ...
        if dir.join("llama-server").exists() {
            return dir;
        }
    }
...
fn sidecar_paths(app: &AppHandle) -> sw_local_llm::SidecarPaths {
    ...
    sw_local_llm::SidecarPaths {
        server_bin: dir.join("llama-server"),
        lib_dir: dir,
    }
}
...
pub fn list_local_llm_models() -> Vec<LlmModelInfo> {
    sw_local_llm::curated().iter().map(info_from_entry).collect()
}
...
#[tauri::command]
pub async fn delete_local_llm_model(app: AppHandle, model_id_or_repo: String) -> Result<(), String> {
    let entry = sw_local_llm::resolve(&model_id_or_repo)
        .ok_or_else(|| format!("unknown model: {model_id_or_repo}"))?;
...
pub fn set_responder_preference(
    ...
    let normalized = if preference == "local" { "local" } else { "external" };
    let prefs = {
        let mut state = data.lock().map_err(|_| "state poisoned".to_string())?;
...
impl LocalTurnQueue {
    pub fn new() -> Self {
        ...
        let pending: Arc<std::sync::Mutex<Vec<String>>> = Arc::new(std::sync::Mutex::new(Vec::new()));
        let draining = Arc::clone(&pending);
        tauri::async_runtime::spawn(async move {
            while let Some((session_id, turn)) = rx.recv().await {
                turn.await;
                if let Some(session_id) = session_id {
                    if let Ok(mut guard) = draining.lock() {
                        if let Some(idx) = guard.iter().position(|s| *s == session_id) {
                            guard.remove(idx);
                        }
                    }
                }
    ...
    pub fn enqueue(&self, turn: impl std::future::Future<Output = ()> + Send + 'static) {
        let _ = self.tx.send((None, Box::pin(turn)));
    }
    ...
    pub fn enqueue_for(
        ...
        let _ = self
            .tx
            .send((Some(session_id.to_string()), Box::pin(turn)));
    }

    pub fn is_pending(&self, session_id: &str) -> bool {
        self.pending
            .lock()
            .map(|guard| guard.iter().any(|s| s == session_id))
            .unwrap_or(false)
    }
}

#[tauri::command]
pub fn local_turn_pending(app: AppHandle, session_id: String) -> bool {
    app.state::<LocalTurnQueue>().is_pending(&session_id)
}
