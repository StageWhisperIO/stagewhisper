...
use sw_local_llm::types::{GenerationChunk, InferenceParams, LlmError, ModelEntry, ModelKind};
...
impl LocalLlmRuntime {
        ...
        F: FnMut(GenerationChunk) + Send,
    {
        ...
        let first = {
            ...
            engine.infer(system, prompt, params, &mut tracked).await
        };
        ...
        match first {
            ...
            Err(err) => {
                ...
                let engine = guard
                    ...
                    .as_ref()
                    .ok_or_else(|| LlmError::Load("engine not loaded".to_string()))?;
                engine.infer(system, prompt, params, &mut on_token).await
            }
        }
    }
...
pub async fn generate_reply(
    ...
    runtime
        .generate(&sidecar, &entry, system, prompt, &params, |_chunk| {})
        .await
        .map_err(|e| e.to_string())
}
