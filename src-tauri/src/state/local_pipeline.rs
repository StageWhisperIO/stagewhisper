...
pub async fn get_model_status(
    ...
    let ready = tokio::task::spawn_blocking({
        let dir = dir.clone();
        move || sw_audio_recording::download::models_ready(&dir)
    })
    .await
...
pub async fn download_models(app: AppHandle) -> Result<(), String> {
    ...
    let already_ready =
        tokio::task::spawn_blocking(move || sw_audio_recording::download::models_ready(&dir_check))
            .await
...
pub async fn check_models_ready(data: State<'_, Mutex<AppState>>) -> Result<bool, String> {
    let dir = model_dir();
    let ready =
        tokio::task::spawn_blocking(move || sw_audio_recording::download::models_ready(&dir))
            .await
            .map_err(|e| format!("model check failed: {e}"))?;
    if let Ok(mut state) = data.lock() {
        state.models_ready = ready;
