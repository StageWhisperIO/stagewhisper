...
pub async fn request_microphone_permission(app: AppHandle) -> Result<PermissionState, String> {
    ...
    let _ = app.emit(PERMISSIONS_STATUS_CHANGED_EVENT, &build_permissions_status());
    Ok(state)
}

#[tauri::command]
pub async fn request_screen_recording_permission(app: AppHandle) -> Result<PermissionState, String> {
