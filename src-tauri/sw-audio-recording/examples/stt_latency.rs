...
fn main() -> anyhow::Result<()> {
    ...
    let wav_path = PathBuf::from(args.next().expect("usage: stt_latency <wav-16k-mono> [speed]"));
        ...
        for event in pipeline.process_audio_chunk(block) {
            match event {
                PipelineEvent::Transcript { text, is_final } => {
                    eprintln!(
                        "[stt-harness] transcript final={is_final} audio_pos={audio_pos_s:.2}s wall={:.2}s chars={}",
    ...
    for event in pipeline.flush() {
        if let PipelineEvent::Transcript { text, is_final } = event {
            eprintln!("[stt-harness] flush transcript final={is_final} chars={}", text.len());
        }
    }
    Ok(())
}
