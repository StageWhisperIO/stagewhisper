...
const ACCEL_EP_NAME: &str = "CoreML";
#[cfg(windows)]
const ACCEL_EP_NAME: &str = "DirectML";
...
impl Encoder {
    ...
    pub fn load(
        ...
        let session = if use_accel {
            match Self::try_load_with_accel(path, verbose, cache_dir) {
                Ok(s) => {
                    if verbose {
                        println!("Encoder loaded with {ACCEL_EP_NAME} execution provider");
                    }
                    s
                }
                Err(msg) => {
                    if verbose {
                        eprintln!("{ACCEL_EP_NAME} failed for encoder ({msg}), falling back to CPU...");
                    }
    ...
    #[cfg(target_vendor = "apple")]
    fn try_load_with_accel(
        path: &Path,
        verbose: bool,
        cache_dir: Option<&Path>,
    ) -> std::result::Result<Session, String> {
        Self::try_load_with_coreml(path, verbose, cache_dir)
    }
    ...
    fn try_load_with_accel(
        ...
        let _ = cache_dir;
        Self::try_load_with_directml(path, verbose)
    }

    #[cfg(windows)]
    fn try_load_with_directml(path: &Path, verbose: bool) -> std::result::Result<Session, String> {
