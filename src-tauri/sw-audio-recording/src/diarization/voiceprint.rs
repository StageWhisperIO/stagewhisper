...
impl VoicePrintStore {
    ...
    pub fn persist(&self) -> Result<()> {
        ...
        let tmp_path = unique_tmp_path(&self.path);
        let result = write_encrypted_via_tmp(&tmp_path, &self.path, &encrypted);
        if result.is_err() {
...
fn write_encrypted_via_tmp(tmp: &Path, path: &Path, encrypted: &[u8]) -> Result<()> {
    std::fs::write(tmp, encrypted)
        .with_context(|| format!("write voiceprint tmp: {}", tmp.display()))?;
    std::fs::rename(tmp, path)
        .with_context(|| format!("rename voiceprint store: {}", path.display()))
}
...
mod tests {
    ...
    fn unique_tmp_path_never_returns_the_same_path_twice() {
        let base = PathBuf::from("/does/not/matter/voiceprints.bin");
    ...
    #[test]
    fn persist_removes_the_leftover_temp_file_when_rename_fails() {
        let path = temp_store_path("failed_rename.bin");
        let _ = std::fs::remove_file(&path);
        ...
        let leftover_tmp_files: Vec<_> = std::fs::read_dir(path.parent().unwrap())
            .unwrap()
            .filter_map(|entry| entry.ok())
            .map(|entry| entry.path())
            .filter(|p| {
                p.to_string_lossy().contains(&format!(
                    "{}.tmp.",
