...
/// Identity of the artifact set this build pins: repo, revision, filename and digest of
/// every file `download_model` installs. A build that changes any pin changes this string.
pub fn pinned_revision() -> String {
    let mut hasher = Sha256::new();
    for file in FP16_FILES {
        hasher.update(file.repo.as_bytes());
        hasher.update(file.revision.as_bytes());
        hasher.update(file.filename.as_bytes());
        hasher.update(file.sha256.as_bytes());
    }
    hasher.update(crate::vad::SILERO_VAD_SHA256.as_bytes());
...
async fn download_file(
    ...
    let tmp_path = unique_tmp_path(dest_path);
    ...
    while let Some(chunk) = stream.next().await {
        let chunk = chunk
            ...
            .with_context(|| format!("Error downloading {}", spec.filename))?;
        hasher.update(&chunk);
    ...
    file.sync_all().await.inspect_err(|_| {
        let _ = std::fs::remove_file(&tmp_path);
    })?;
    drop(file);
...
mod tests {
    ...
    fn unique_tmp_path_never_returns_the_same_path_twice() {
        let base = PathBuf::from("/does/not/matter/encoder-model.fp16.onnx");
