...
pub async fn ensure_vad_model(model_dir: &std::path::Path) -> Result<std::path::PathBuf> {
    ...
    let tmp_path = unique_tmp_path(&vad_path);
    let mut file = tokio::fs::File::create(&tmp_path).await?;
    file.write_all(&bytes).await.inspect_err(|_| {
    ...
    drop(file);
    tokio::fs::rename(&tmp_path, &vad_path)
        .await
        .inspect_err(|_| {
            let _ = std::fs::remove_file(&tmp_path);
        })?;
...
impl VadSegmenter {
    pub fn new(config: VadConfig) -> Self {
        ...
        let adaptive_silence_chunks =
            ((config.adaptive_silence_ms / chunk_ms).max(1) as usize).min(base_silence_chunks);
...
mod tests {
    ...
    fn unique_tmp_path_never_returns_the_same_path_twice() {
        let base = std::path::PathBuf::from("/does/not/matter/silero_vad.onnx");
