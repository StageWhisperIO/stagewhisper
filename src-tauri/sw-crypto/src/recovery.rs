...
pub fn wrap_amk_with_passphrase(
    amk: &[u8; 32],
    passphrase: &str,
) -> Result<(Vec<u8>, Vec<u8>, Vec<u8>, RecoveryKdfParams), CryptoError> {
    let mut salt = [0u8; 16];
    ...
    Ok((wrapped, nonce.to_vec(), salt.to_vec(), params))
}
...
mod tests {
    ...
    fn wrap_unwrap_roundtrip() {
        let amk = [0x77u8; 32];
        let (wrapped, nonce, salt, params) =
            wrap_amk_with_passphrase(&amk, "correct horse battery staple").unwrap();
    ...
    fn wrong_passphrase_fails() {
        let amk = [0x88u8; 32];
        let (wrapped, nonce, salt, params) = wrap_amk_with_passphrase(&amk, "phrase one").unwrap();
        let err = unwrap_amk_with_passphrase("phrase two", &salt, &nonce, &wrapped, &params);
        assert!(err.is_err());
    }
