...
fn select_gguf_files(tree: Vec<HfTreeEntry>) -> Vec<PlannedFile> {
    ...
    let mut planned = key.and_then(|k| groups.remove(&k)).unwrap_or_default();
...
async fn download_file(
    ...
    while let Some(chunk) = stream.next().await {
        ...
        let chunk = chunk
            ...
            .map_err(|e| LlmError::Download(format!("error downloading {}: {e}", file.path)))?;
        out.write_all(&chunk)
            .await
            .inspect_err(|_| {
                let _ = std::fs::remove_file(&tmp_path);
...
mod tests {
    ...
    fn unique_tmp_path_never_returns_the_same_path_twice() {
        let base = PathBuf::from("/does/not/matter/model.gguf");
    ...
    fn select_gguf_prefers_q4_k_m_and_groups_shards() {
        ...
        let picked: Vec<String> = select_gguf_files(tree)
            .into_iter()
            .map(|f| f.path)
            .collect();
        assert_eq!(
    ...
    fn hf_cache_snapshot_resolves_main_ref() {
        ...
        let found =
            hf_cache_snapshot_in(std::slice::from_ref(&base), "unsloth/Qwen3.6-35B-A3B-GGUF");
        assert_eq!(found.as_deref(), Some(snap.as_path()));
        assert!(hf_cache_snapshot_in(std::slice::from_ref(&base), "missing/repo").is_none());
        let _ = std::fs::remove_dir_all(&base);
    }
}
