    ...
    pub async fn count_tokens(&self, text: &str) -> Result<usize, LlmError> {
        ...
        if !response.status().is_success() {
            ...
            return Err(LlmError::Inference(format!(
                "llama-server tokenize failed: {detail}"
            )));
        }

        let payload: TokenizeResponse = response
            .json()
            .await
            .map_err(|e| LlmError::Inference(e.to_string()))?;
        Ok(payload.tokens.len())
    }
    ...
    F: FnMut(GenerationChunk),
{
    ...
    if saw_reasoning && full.trim().is_empty() {
        return Err(LlmError::ReasoningBudgetExhausted(
            "model spent the whole token budget on reasoning and returned no answer".to_string(),
        ));
    }
...
fn build_request_messages(system: Option<&str>, history: &[ChatMessage]) -> Vec<serde_json::Value> {
...
mod tests {
    ...
    fn finish_reason_is_terminal() {
        let parsed = parse_sse_line(r#"data: {"choices":[{"delta":{},"finish_reason":"stop"}]}"#);
        assert!(parsed.finished);
    }
    ...
    #[tokio::test]
    async fn reasoning_only_stream_reports_an_exhausted_budget() {
        let result = super::read_completion_stream(
            futures_util::stream::iter(reasoning_only_chunks()),
            std::time::Duration::from_secs(5),
            false,
        ...
        assert!(matches!(
            result,
            Err(super::LlmError::ReasoningBudgetExhausted(_))
        ));
    }
