...
struct MockHost {
    ...
    use_hosted_summary: bool,
    summary_was_sent: Mutex<bool>,
...
impl MockHost {
    fn new(send_outcome: SendOutcome) -> Self {
        Self {
            ...
            use_hosted_summary: false,
            summary_was_sent: Mutex::new(false),
    ...
    fn new_local(send_outcome: SendOutcome) -> Self {
        Self {
            ...
            ..Self::new(send_outcome)
        }
    }

    fn new_hosted_without_a_paired_assistant(send_outcome: SendOutcome) -> Self {
        Self {
            has_relay: false,
            use_hosted_summary: true,
            ..Self::new(send_outcome)
        }
    }

    fn summary_was_sent(&self) -> bool {
        *self.summary_was_sent.lock().unwrap()
    }

    fn reported_error(&self) -> Option<String> {
        self.reported_error.lock().unwrap().clone()
    }
...
impl NotesHost for MockHost {
    async fn send_summary(
        ...
        *self.summary_was_sent.lock().unwrap() = true;
    ...
    fn settings(&self) -> NotesSettings {
        NotesSettings {
            ...
            use_hosted_summary: self.use_hosted_summary,
        }
    }
    ...
    fn emit(&self, event: NotesEvent) {
        match event {
            ...
            NotesEvent::NotesError { message, .. } => {
                *self.reported_error.lock().unwrap() = Some(message)
            }
            _ => {}
        }
    }
...
#[tokio::test]
async fn a_signals_session_without_a_paired_assistant_still_gets_its_summary_requested() {
    let store = temp_store("hosted-without-relay");
    let session_id = Uuid::new_v4();
    seed_completed_summary(&store, session_id);
    let host = MockHost::new_hosted_without_a_paired_assistant(SendOutcome::Succeed);

    run_regeneration(&store, session_id, &host).await;

    assert!(
        host.summary_was_sent(),
        "a hosted session should not be gated behind having a paired assistant",
    );
    let record = store.load(&session_id.to_string()).unwrap().unwrap();
    assert_eq!(
        record.notes_pending_root_message_id,
        Some(host.pending_id())
    );
    assert!(host.reported_error().is_none());
}

#[tokio::test]
async fn a_session_with_neither_a_paired_assistant_nor_signals_is_rejected_before_sending() {
    let store = temp_store("no-relay-no-hosted");
    let session_id = Uuid::new_v4();
    seed_completed_summary(&store, session_id);
    let host = MockHost {
        has_relay: false,
        ..MockHost::new(SendOutcome::Succeed)
    };

    run_regeneration(&store, session_id, &host).await;

    assert!(!host.summary_was_sent());
    assert_eq!(
        host.reported_error().as_deref(),
        Some("Relay not configured")
    );
    let record = store.load(&session_id.to_string()).unwrap().unwrap();
    assert_eq!(
        record.notes_markdown.as_deref(),
