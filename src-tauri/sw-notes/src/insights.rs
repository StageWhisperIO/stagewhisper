...
pub fn append_missing_insights(target: &mut Vec<InsightNote>, incoming: &[InsightNote]) -> bool {
    ...
    for note in incoming {
        let exists = target.iter().any(|existing| {
            existing.offset_ms == note.offset_ms && existing.message == note.message
        });
        if !exists {
    ...
    fn renders_suggestion_with_utterance_quote() {
        let segments = vec![
            ...
            segment(
                ...
                "We are mostly on spreadsheets right now.",
            ),
        ];
        let insights = vec![note(
            134_000,
            "Ask what their current process costs",
            Some(1),
        )];
