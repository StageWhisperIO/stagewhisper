...
fn clean_title(line: &str) -> String {
    let unwrapped = line.trim_matches(|c| matches!(c, '*' | '_' | '`' | '"' | '#')).trim();
...
impl SessionStore {
            ...
            match status {
                "completed" => {
                    if let Ok(Some(live)) = self.read_live_raw(session_id) {
                        crate::insights::append_missing_insights(&mut record.insights, &live.insights);
                    }
                    let mut notes = content.to_string();
                    if let Some(section) =
                        crate::insights::render_suggestions_section(&record.insights, &record.segments)
                    {
                        notes.push_str("\n\n");
                        notes.push_str(&section);
                    }
                    record.notes_markdown = Some(notes);
    ...
    pub fn live_append_insight(
        ...
        let Some(record) = self.read_live_raw(session_id)? else {
            ...
            return self.push_insight_session_locked(session, severity, trimmed, segment_index, now_ms);
        };
    ...
    fn push_insight_session_locked(
        ...
        Self::refresh_suggestions_section(&mut record);
        self.write_raw(&record)
    }

    fn refresh_suggestions_section(record: &mut SessionRecord) {
    ...
    pub fn live_merge_insights_then_delete(&self, session_id: &str) -> Result<(), StoreError> {
        ...
        if let Some(live) = self.read_live_raw(session_id)? {
            if crate::insights::append_missing_insights(&mut session.insights, &live.insights) {
                Self::refresh_suggestions_section(&mut session);
...
mod tests {
    ...
    #[test]
    fn live_insights_flow_into_notes_section() {
        let store = temp_store([0x77; 32], "insights_flow");
        store.live_begin("sess-i", 1_000).unwrap();
        store
            .live_append_segment("sess-i", TranscriptSource::Others, "we use spreadsheets", 5_000)
            .unwrap();
        store
            .live_append_insight("sess-i", "green", "Ask about their process cost", Some(0), 61_000)
            .unwrap();
        store
            .live_append_insight("sess-i", "green", "Ask about their process cost", Some(0), 62_000)
            .unwrap();
        ...
        let notes = saved.notes_markdown.unwrap();
        assert!(notes.starts_with("# Call Notes"));
        assert!(notes.contains("## Suggestions"));
        assert!(notes.contains("- **01:00** Ask about their process cost"));
        assert!(notes.contains("> You: \"hello world\""));
        let chat_reply = saved.chat.iter().find(|m| m.id == "task-1").unwrap();
        assert!(!chat_reply.content.contains("## Suggestions"));
    }
    ...
    #[test]
    fn merge_after_completed_notes_rebuilds_suggestions_section() {
        let store = temp_store([0x7d; 32], "insights_after_notes");
        store.live_begin("sess-ln", 1_000).unwrap();
        store
            .live_append_segment("sess-ln", TranscriptSource::Others, "we use spreadsheets", 5_000)
            .unwrap();
        ...
        let notes = saved.notes_markdown.unwrap();
        assert!(notes.starts_with("# Call Notes"));
        assert!(notes.contains("Arrived after summary"));
        ...
        let notes = saved.notes_markdown.unwrap();
        assert!(notes.contains("Arrived after summary"));
        assert!(notes.contains("Even later insight"));
        assert_eq!(notes.matches("## Suggestions").count(), 1);
    }
    ...
    fn recovery_merge_unions_live_insights_with_saved_session() {
        ...
        store
            .live_append_segment("sess-rec", TranscriptSource::Others, "we use spreadsheets", 5_000)
            .unwrap();
        store
        ...
        store
            .live_append_insight("sess-rec", "green", "Post-snapshot insight", Some(0), 12_000)
            .unwrap();
    ...
    fn late_insights_survive_finalize_handoff() {
        ...
        store
            .live_append_segment("sess-late", TranscriptSource::Others, "we use spreadsheets", 5_000)
            .unwrap();
        store.save(&sample("sess-late")).unwrap();
        store
            .live_append_insight("sess-late", "green", "Landed before cleanup", Some(0), 6_000)
            .unwrap();
        ...
        let notes = store
            ...
            .notes_markdown
            .unwrap();
        assert!(notes.contains("Landed before cleanup"));
        assert!(notes.contains("Landed after cleanup"));
    }
    ...
    fn record_reply_merges_pending_live_insights() {
        ...
        store
            .live_append_segment("sess-rm", TranscriptSource::Others, "we use spreadsheets", 5_000)
            .unwrap();
        ...
        let notes = store
            ...
            .notes_markdown
            .unwrap();
        assert!(notes.contains("Arrived mid-handoff"));
    ...
    fn insight_keeps_dispatch_time_segment_index() {
        ...
        store
            .live_append_segment("sess-dc", TranscriptSource::Others, "we use spreadsheets", 5_000)
            .unwrap();
        let dispatch_segment_index = Some(0);
        store
            .live_append_segment("sess-dc", TranscriptSource::You, "unrelated later remark", 8_000)
            .unwrap();
        store
            .live_append_insight(
                "sess-dc",
