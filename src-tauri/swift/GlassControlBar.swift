...
struct GlassControlBarView: View {
    ...
    var body: some View {
        ...
        .animation(.smooth(duration: 0.35), value: state.isReady)
    }
}
...
struct FallbackControlBarView: View {
    ...
    var body: some View {
        ...
        .animation(.easeInOut(duration: 0.25), value: state.isReady)
    }
}
...
func updateState(isListening: Bool, isReady: Bool) {
    let apply = {
        guard #available(macOS 14, *) else { return }
        sharedState.isListening = isListening
        sharedState.isReady = isReady
