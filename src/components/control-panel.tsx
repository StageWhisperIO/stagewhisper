...
import { useLocalLlmReady } from "../hooks/local-llm-ready";
...
export function ControlPanel() {
  ...
  const sttReady = useModelReady();
  const { ready: localLlmReady, prefersLocal } = useLocalLlmReady();
  ...
  const openModelSettings = useCallback(async () => {
    try {
      await invoke("open_settings_window");
      await emit("settings-navigate", { section: "models" });
    } catch (error) {
      console.error("[ControlPanel] failed to open model settings", error);
    }
  }, []);

  const reasoningReady = prefersLocal ? localLlmReady === true : connected === true;
  const sttMissing = sttReady === false;
  const reasoningMissing =
    connected !== null && localLlmReady !== null && !reasoningReady;

  return (
    <div
      className={`flex ${isWindows ? "my-[6px] h-[36px] rounded-full bg-[#0b0e16f0] ring-1 ring-white/10" : "h-full"} w-full select-none items-center justify-center gap-2.5 px-3`}
    ...
    >
      {sttMissing ? (
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs font-semibold text-white shadow-[inset_0_1px_0_#ffffff40,0_2px_6px_#00000059] backdrop-blur-md transition-colors hover:bg-white/20 hover:border-white/30 active:bg-white/25 disabled:opacity-50 disabled:hover:bg-white/10"
        >
          {isListening ? (
