...
export function ConnectionSection() {
  ...
  useEffect(() => {
    if (topology === "remote") setShowCallback(true);
  }, [topology]);
    ...
    <main className="flex h-full w-full flex-col overflow-y-auto px-8 pb-8 pt-2 text-body">
          ...
          <div className="space-y-6">
            ...
            {topology === "remote" && (
              <p className="-mt-2 text-xs leading-relaxed text-muted">
                ...
                <code className="font-mono text-body">tailscale status</code> on it), not localhost.
            ...
            {topology === "remote" && (
              <div className="rounded-lg border border-accent/20 bg-accent/5 px-4 py-3">
                <p className="text-xs leading-relaxed text-body">
                  A remote assistant also needs a return path so replies can reach this Mac. Set it
                  up under <span className="font-medium text-accent">Remote assistant callback</span>{" "}
                  below.
                </p>
              </div>
            )}
      ...
      <section className="mt-8 border-t border-rule pt-6">
        <button
          ...
          className="text-sm font-medium text-body transition hover:text-accent"
        >
          {showCallback ? "Hide remote callback setup" : "Remote assistant callback (advanced)"}
        </button>

        {showCallback && (
          <div className="mt-4 space-y-5">
            <p className="text-sm leading-relaxed text-muted">
              Only needed when your assistant runs on another machine. The reply server always
              listens on 127.0.0.1. Expose that port over your private network (for example with{" "}
              <code className="font-mono text-xs text-body">tailscale serve</code>), then tell
              StageWhisper which address to hand your assistant for replies.
            </p>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {callbackStatus === "saving" ? "Saving..." : "Save"}
              </button>
