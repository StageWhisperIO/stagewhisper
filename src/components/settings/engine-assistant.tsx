...
export function EngineAssistantCard() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const hasCallback =
        Boolean(settings.callbackUrl && settings.callbackUrl.trim().length > 0) ||
        settings.callbackPort != null;
      if (hasCallback) setTopology("remote");
    } catch (err) {
  ...
  const callbackSection = (
    <div className="space-y-4 rounded-lg border border-rule-strong px-4 py-4">
      <div>
        <p className="text-sm font-medium text-heading">Return path</p>
  ...
  if (verified) {
    return (
      <div className="space-y-6">
        ...
        {topology === "remote" && callbackSection}
      ...
      <div className="space-y-5">
        ...
        {probeStatus === "error" ? (
          <p className="text-sm leading-relaxed text-body">
            Couldn't reach your assistant relay. Make sure the assistant is running and, if it's on a
            remote host, that your tunnel is up. Then check again.
          </p>
        ) : (
          <p className="text-sm leading-relaxed text-body">
            Approve this device once on your assistant host, then confirm below. Until then it can't
            respond, so recording stays disabled.
          </p>
        )}

        {probeStatus === "checking" && <p className="text-sm text-muted">Checking the connection...</p>}
            ...
            {probeReply ? (
              <>
                <p className="text-xs font-medium text-accent">
                  {`Your ${provider.human_name} assistant replied with your pairing code. Run the command it shows on the host:`}
                </p>
                <div className="group relative">
              ...
              <p className="text-xs font-medium text-accent">
                {`Checking with your ${provider.human_name} assistant for a pairing code...`}
              </p>
            )}
          </div>
        )}

        {probeStatus === "error" && probeError && <p className="text-sm text-danger">{probeError}</p>}
    ...
    <div className="space-y-6">
      ...
      {topology === "remote" && (
        <p className="-mt-2 text-xs leading-relaxed text-muted">
          ...
          <code className="font-mono text-body">tailscale status</code> on it), not localhost. The
          default code points at 127.0.0.1, which only works on the same machine.
        </p>
      )}
