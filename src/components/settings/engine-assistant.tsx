...
interface ProviderInfo {
  ...
  pair_code_label: string;
  needs_host_address: boolean;
  install_note?: string;
  pair_code_note?: string;
}
...
const PROVIDERS: ProviderInfo[] = [
  {
    ...
    pair_code_label: "2. Generate a pairing code on your OpenClaw host:",
    needs_host_address: true,
  },
  {
    kind: "hermes",
    human_name: "Hermes",
    install_command:
      "uv pip install --python <hermes-venv>/bin/python hermes-platform-stagewhisper && <hermes-venv>/bin/stagewhisper-hermes-install",
    pair_code_command: "<hermes-venv>/bin/stagewhisper-hermes pair-code",
    pair_code_label: "2. Generate a pairing code on your Hermes host:",
    needs_host_address: true,
    install_note:
      "Replace <hermes-venv> with the environment your Hermes gateway runs from. It has to be that same one, or the gateway can't load the plugin.",
    pair_code_note:
      "Setting up a whole team on one Linux host? Run sudo <hermes-venv>/bin/stagewhisper-hermes provision <name> instead. It gives each person their own assistant and their own pairing code.",
  },
];
...
export function EngineAssistantCard() {
  ...
  return (
    <div className="space-y-6">
      ...
      />
      {provider.install_note && (
        <p className="-mt-2 text-xs leading-relaxed text-muted">{provider.install_note}</p>
      )}

      <CommandCard
        label={
          provider.needs_host_address && topology === "remote"
            ? `2. Generate a pairing code on the host, pointing at the address ${thisMachine} reaches it on:`
            : provider.pair_code_label
        }
        command={
          provider.needs_host_address && topology === "remote"
      ...
      {provider.pair_code_note && (
        <p className="-mt-2 text-xs leading-relaxed text-muted">{provider.pair_code_note}</p>
      )}
