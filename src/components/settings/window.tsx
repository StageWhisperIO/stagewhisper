...
type SidebarSection = "engine" | "permissions" | "library" | "about";

function normalizeSection(section: string | undefined): SidebarSection | null {
  switch (section) {
    case "engine":
    case "connection":
    case "models":
...
export function SettingsWindow() {
  ...
  const [activeSection, setActiveSection] = useState<SidebarSection>("engine");
  ...
  useEffect(() => {
    ...
    const unlistenNavigate = listen<{ section: string }>("settings-navigate", (event) => {
      const section = normalizeSection(event.payload?.section);
      if (section) {
  ...
  return (
    <main className="settings-surface flex h-full w-full bg-page text-body">
      <aside className="flex w-56 shrink-0 flex-col border-r border-rule bg-panel">
        <div className={`${isWindows ? "h-4" : "h-12"} shrink-0`} data-tauri-drag-region />
      ...
      <div className="flex flex-1 flex-col overflow-hidden bg-page">
        {!isWindows && <div className="h-8 shrink-0" data-tauri-drag-region />}
