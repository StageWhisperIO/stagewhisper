...
export function useLocalLlm() {
  ...
  useEffect(() => {
    ...
    listen<string>("local-llm-download-error", (e) => {
      ...
      setDownloadProgress(null);
      setDownloadError(
        userFacingError(e.payload, "Couldn't download the model. Check your connection and try again."),
      );
      void refresh();
    }).then((fn) => { if (dead) fn(); else cleanups.push(fn); });
  ...
  const downloadModel = useCallback(
    async (modelIdOrRepo: string, hfToken?: string) => {
      ...
      try {
        ...
        setDownloading(false);
        setDownloadError(
          userFacingError(err, "Couldn't download the model. Check your connection and try again."),
        );
      }
    },
    [],
  ...
  const deleteModel = useCallback(
    async (modelIdOrRepo: string) => {
      try {
        ...
        setDownloadError(userFacingError(err, "Couldn't delete the model. Try again."));
      }
    },
    [refresh],
  );
  ...
  const selectModel = useCallback(
    async (modelIdOrRepo: string) => {
      try {
        ...
        setDownloadError(userFacingError(err, "Couldn't switch to that model. Try again."));
      }
    },
    [refresh],
  );
  ...
  const setPrimary = useCallback(
    async (preference: "external" | "local") => {
      try {
        ...
        setDownloadError(userFacingError(err, "Couldn't switch your default assistant. Try again."));
      }
    },
    [refresh],
  );
  ...
  const useLocalFolder = useCallback(async () => {
    ...
    try {
      ...
      setDownloadError(
        userFacingError(err, "Couldn't use that folder. Make sure it contains a supported model file and try again."),
      );
      return null;
    }
  }, [refresh]);
  ...
  const useCachedModel = useCallback(
    async (repoId: string) => {
      ...
      try {
        ...
        setDownloadError(userFacingError(err, "Couldn't use the cached model. Try downloading it again."));
        return null;
      }
    },
    [refresh],
